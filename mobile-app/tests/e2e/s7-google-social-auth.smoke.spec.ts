...
describe("S7 Google social auth smoke", () => {
  it("accepts a Google deep-link callback and stays on the app shell", async () => {
    ...
    mockUrlHandler.current?.({ url: "topela://auth/google/callback?status=success&provider=google&next=%2Fexplore&sessionToken=smoke-session-token" });
