...
describe("US5 Google social cancel acceptance", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("keeps the user signed out when a Google callback is cancelled", async () => {
    ...
    mockUrlHandler.current?.({ url: "topela://auth/google/callback?status=cancelled&provider=google&next=%2Fmy-hub" });
