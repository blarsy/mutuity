...
describe("US5 social link-required acceptance", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("routes users back to the login screen for a link_confirmation_required callback", async () => {
    ...
    mockUrlHandler.current?.({ url: "topela://auth/google/callback?status=link_confirmation_required&provider=google&next=%2Fmy-hub&error=link-required" });
