...
describe("US5 social register-required acceptance", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("routes users into the registration flow for a register_required callback", async () => {
    ...
    mockUrlHandler.current?.({ url: "topela://auth/google/callback?status=register_required&provider=google&next=%2Fmy-hub&name=Jane%20Doe&email=jane%40example.com&providerSubject=subject-123" });
