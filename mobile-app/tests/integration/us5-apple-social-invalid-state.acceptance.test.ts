...
describe("US5 Apple social invalid state acceptance", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("keeps the user signed out when an Apple callback has invalid state", async () => {
    ...
    mockUrlHandler.current?.({ url: "topela://auth/apple/callback?status=error&provider=apple&next=%2Fmy-hub&error=invalid-state" });

    await waitFor(() => {
      expect(screen.getByTestId("auth-login-screen")).toBeTruthy();
    });

    expect(screen.getByText(/continue with apple/i)).toBeTruthy();
  });
});
