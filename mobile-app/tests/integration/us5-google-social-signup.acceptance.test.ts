...
describe("US5 Google social signup acceptance", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("opens the registration completion flow for a Google signup callback", async () => {
    ...
    const callbackUrl = "topela://auth/google/callback?status=register_required&provider=google&next=%2Fmy-hub&name=Jane%20Doe&email=jane%40example.com&providerSubject=subject-123";
    ...
    expect(screen.getByTestId("auth-register-full-name").props.value).toBe("Jane Doe");
    expect(screen.getByTestId("auth-register-email").props.value).toBe("jane@example.com");
