...
begin
  ...
  select
    ...
    a.avatar_url as other_account_avatar_url,
    coalesce(
      (select image_url from unnest(r.image_urls) as image_url where image_url is not null and image_url <> '' limit 1),
      (select image_url from unnest(n.image_urls) as image_url where image_url is not null and image_url <> '' limit 1)
  ...
  left join app_public.resource r on b.conversation_kind = 'resource' and r.id = b.context_id
  left join app_public.need n on b.conversation_kind = 'need' and n.id = b.context_id
  where (
    v_pattern is null
