...
-- Mark listing functions STABLE so PostGraphile exposes them as GraphQL queries
-- rather than mutations. The functions are pure reads that depend only on database
-- state for the current account.
...
-- Revoke direct SELECT on the internal summary view from regular user roles.
-- list_chat_conversations uses SECURITY DEFINER and filters by account, so
-- direct access to the view would bypass per-account scoping.
revoke select on app_public.chat_conversation_summary from identified_account;
