...
begin
  ...
  if nullif(btrim(password), '') is null then
    raise exception using message = 'Password is required';
  end if;
...
grant execute on function app_public.register_local_account_with_social_identity(
  text,
  text,
  text,
  text,
...
comment on function app_public.register_local_account_with_social_identity(
  text,
  text,
  text,
  text,
  ...
  text,
  bigint
) is '@name registerLocalAccountWithSocialIdentity';
