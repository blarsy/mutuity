...
begin
  ...
  from (
    select distinct lower(btrim(raw_label)) as label
    from unnest(coalesce(publish_resource.category_labels, array[]::text[])) as raw_label
    where nullif(btrim(raw_label), '') is not null
  ) normalized_labels;
