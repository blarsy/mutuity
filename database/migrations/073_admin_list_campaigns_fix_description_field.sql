-- Correct the admin_list_campaigns payload mapping introduced in migration 072.
-- The campaign table does not have a `description` column; the descriptive text
-- used by the product is stored in `theme`.
...
begin
  ...
  where (
    ...
    or coalesce(c.theme,                     '') ilike ('%' || v_search || '%')
