...
declare
  ...
  v_has_verified_email_match boolean;
begin
  ...
  if p_provider_email_verified = true and v_email_normalized <> '' then
    ...
    into v_has_verified_email_match;

    if v_has_verified_email_match then
      return query
...
comment on function app_private.resolve_account_for_external_identity(text, text, text, boolean)
  is 'Resolves by provider subject first; requests explicit link only when a verified identity already exists for the provider email.';
