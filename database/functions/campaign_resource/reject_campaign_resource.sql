create or replace function app_public.reject_campaign_resource(campaign_id uuid, resource_id uuid)
...
begin
  ...
  where cr.campaign_id = reject_campaign_resource.campaign_id
    and cr.resource_id = reject_campaign_resource.resource_id
  for update of cr;
  ...
  set status = 'rejected',
      ...
      acted_at = now()
  where app_public.campaign_resource.campaign_id = reject_campaign_resource.campaign_id
    and app_public.campaign_resource.resource_id = reject_campaign_resource.resource_id
...
grant execute on function app_public.reject_campaign_resource(uuid, uuid) to identified_account, admin;

comment on function app_public.reject_campaign_resource(uuid, uuid)
  is '@name rejectCampaignResource';
